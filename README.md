...
정기 근무 말고 즉석으로 시킬 때는 **이슈**를 씁니다. 이슈 하나가 칸반 카드 한 장입니다.

1. [새 이슈](https://github.com/heekeunlee/ai_crew/issues/new/choose) → 에이전트를 고릅니다
2. 지시를 씁니다
3. 올립니다 — 라벨(`agent:<id>`)은 템플릿이 알아서 붙입니다

Actions로 돌 때는 라벨이 붙는 즉시, 로컬로 돌 때는 2분 안에 집어갑니다.
끝나면 그 이슈에 산출물 링크를 댓글로 달고 닫습니다.

기계가 필요 없습니다 — 휴대폰 브라우저나 GitHub 앱이면 됩니다.

**지시는 그 근무의 "할 일"만 대체합니다.** 출력 형식과 마지막 블록 규칙은
`agents/<id>/TASK.md`의 것을 그대로 지키므로, 형식까지 바꾸려면 지시에 직접 적어야 합니다.

> 에이전트를 늘리면 `.github/ISSUE_TEMPLATE/`에 파일 하나를 더 만들어야 합니다.
> 로스터를 읽어 자동 생성할 수도 있지만, 이 파일들은 사람이 읽을 안내문이라
> 손으로 쓰는 편이 낫다고 봤습니다. `crew.json` 하나만 고치면 되는 원칙에서
> 벗어나는 유일한 곳입니다.
...
| 위치 | 뜻 |
|---|---|
| 책상 | 지금 근무 중 (모니터 켜짐) — Actions든 mini든 |
